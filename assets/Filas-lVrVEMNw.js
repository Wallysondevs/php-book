...
}`}),e.jsxs("p",{children:["Reescrevendo com fila: o controller só publica ",e.jsx("em",{children:"jobs"})," e responde. O worker que roda em segundo plano executa cada job quando puder."]}),e.jsx(a,{filename:"signup-async.php",code:`<?php
declare(strict_types=1);
...
Package operations: 1 install, 0 updates, 0 removals
  - Installing predis/predis (v2.2.2): Extracting archive`}),e.jsx(a,{filename:"producer.php",code:`<?php
declare(strict_types=1);
...
echo "Job enfileirado. Tamanho atual da fila: $tamanho" . PHP_EOL;`,output:"Job enfileirado. Tamanho atual da fila: 1"}),e.jsx(a,{filename:"worker.php",code:`<?php
declare(strict_types=1);
...
function enviarEmail(string $para, string $assunto): void {
    ...
    usleep(500_000);
}`}),e.jsx(s,{user:"dev",host:"php",cwd:"~/projetos/app",command:"php worker.php",output:`[worker] aguardando jobs em fila:emails...
[worker] processando user_id=42
[worker] ok`}),e.jsxs(o,{type:"warning",title:"Cuidado com o BLPOP em loop infinito",children:["Workers PHP vazam memória com o tempo (objetos longos, conexões abertas). Sempre rode com um ",e.jsx("strong",{children:"supervisor"})," (systemd, supervisord) que reinicia o processo a cada N jobs ou X minutos. Veja o capítulo de produção."]}),e.jsx("h2",{children:"Opção 2: Beanstalkd — fila dedicada com prioridades"}),e.jsxs("p",{children:["Beanstalkd é um servidor de filas minimalista, pensado para isso e só isso. Tem prioridades, atraso (",e.jsx("em",{children:"delay"}),"), TTL e ",e.jsx("em",{children:"burying"})," de jobs com falha. Em PHP a biblioteca clássica é ",e.jsx("code",{children:"pda/pheanstalk"}),"."]}),e.jsx(s,{user:"dev",host:"php",cwd:"~/projetos/app",command:"composer require pda/pheanstalk",output:`Using version ^5.0 for pda/pheanstalk
./composer.json has been updated
Installing dependencies (including require-dev) from lock file
  - Installing pda/pheanstalk (v5.0.6): Extracting archive`}),e.jsx(a,{filename:"beanstalk-producer.php",code:`<?php
declare(strict_types=1);
...
echo "Job #{$id->getId()} publicado." . PHP_EOL;`,output:"Job #17 publicado."}),e.jsx(a,{filename:"beanstalk-worker.php",code:`<?php
declare(strict_types=1);
...
$canal->close();
$conn->close();`,output:"Mensagem publicada em emails."}),e.jsx(a,{filename:"rabbit-consumer.php",code:`<?php
declare(strict_types=1);
...
}`}),e.jsx(a,{filename:"src/MessageHandler/EnviarBoasVindasHandler.php",code:`<?php
declare(strict_types=1);
...
final readonly class EnviarBoasVindasHandler
{
    ...
    messenger:
        ...
        routing:
            'App\\Message\\EnviarBoasVindas': async`}),e.jsx(a,{filename:"src/Controller/SignupController.php",code:`<?php
declare(strict_types=1);
...
}`}),e.jsx(s,{user:"dev",host:"php",cwd:"~/projetos/app",command:"php bin/console messenger:consume async -vv",output:`[OK] Consuming messages from transports "async".
