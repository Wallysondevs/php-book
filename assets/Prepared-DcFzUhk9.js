...
var_dump($row !== false);`}),e.jsxs("p",{children:["Se o atacante usa ",e.jsx("code",{children:"?email=' OR 1=1 --&senha=qualquer"}),", a query final vira:"]}),e.jsx(s,{title:"SQL real executado",language:"sql",code:"SELECT * FROM usuarios WHERE email = '' OR 1=1 --' AND senha = 'qualquer'"}),e.jsxs("p",{children:["O ",e.jsx("code",{children:"--"})," comenta o resto, ",e.jsx("code",{children:"OR 1=1"})," casa qualquer linha — login sem senha. A solução é usar ",e.jsx("strong",{children:"prepared statements"}),", que separam ",e.jsx("em",{children:"código SQL"})," de ",e.jsx("em",{children:"valores"}),":"]}),e.jsx(o,{filename:"seguro.php",code:`<?php
declare(strict_types=1);
...
$stmt->execute();
echo $stmt->fetchColumn() . PHP_EOL;`,output:"1"}),e.jsx(o,{filename:"bind-param.php",code:`<?php
declare(strict_types=1);
...
echo "inseridos: " . count($novos) . PHP_EOL;`,output:"inseridos: 3"}),e.jsxs("p",{children:["Repare como o mesmo ",e.jsx("code",{children:"$stmt"})," é executado três vezes mudando apenas as variáveis amarradas — o banco prepara a query uma vez só e ganha em performance."]}),e.jsx("h2",{children:"Tipos PDO::PARAM_*"}),e.jsx("p",{children:"Os tipos suportados que você usa no dia a dia:"}),e.jsx(s,{title:"Tipos PDO::PARAM_*",language:"php",code:`PDO::PARAM_STR    // string (default)
PDO::PARAM_INT    // inteiro
PDO::PARAM_BOOL   // booleano
PDO::PARAM_NULL   // NULL
PDO::PARAM_LOB    // BLOB (binário grande)`}),e.jsx("h2",{children:"Lendo coleções com IN (...)"}),e.jsxs("p",{children:["Uma das pegadinhas: ",e.jsx("strong",{children:"placeholders não expandem arrays"}),". Você precisa gerar a quantidade certa de ",e.jsx("code",{children:"?"})," dinamicamente:"]}),e.jsx(o,{filename:"in.php",code:`<?php
declare(strict_types=1);
...
(
    ...
    [2] => Pão
    [4] => Açúcar 1kg
)`}),e.jsx("h2",{children:"Transações: tudo ou nada"}),e.jsxs("p",{children:["Quando uma operação envolve ",e.jsx("strong",{children:"várias queries que precisam acontecer juntas"})," ","— como debitar de uma conta e creditar em outra — você usa transações. Se uma falha, nada é gravado."]}),e.jsx(o,{filename:"transferencia.php",code:`<?php
declare(strict_types=1);
