...
código: 0 (0 = SUCCESS)`}),e.jsx("h2",{children:"set / get com TTL"}),e.jsxs("p",{children:["O TTL vai como ",e.jsx("strong",{children:"terceiro parâmetro"})," de ",e.jsx("code",{children:"set()"}),". Em segundos, ou"," ",e.jsx("em",{children:"timestamp Unix"})," se for maior que 30 dias (esquisitice histórica do protocolo)."]}),e.jsx(o,{filename:"bin/cache-aside.php",code:`<?php
declare(strict_types=1);
...
Array ( [id] => 99 [nome] => Ada [pontos] => 1024 )`}),e.jsxs(a,{type:"info",title:"Por que checar getResultCode()?",children:[e.jsx("code",{children:"get()"})," retorna ",e.jsx("code",{children:"false"})," tanto quando a chave não existe quanto quando o valor armazenado é literalmente ",e.jsx("code",{children:"false"}),". ",e.jsx("code",{children:"getResultCode()"})," distingue",e.jsx("code",{children:"RES_SUCCESS"})," de ",e.jsx("code",{children:"RES_NOTFOUND"})," sem ambiguidade."]}),e.jsx("h2",{children:"setMulti / getMulti: a virada de jogo de performance"}),e.jsxs("p",{children:["Cada round-trip pela rede custa caro. Em vez de fazer 50 ",e.jsx("code",{children:"get()"})," seguidos, faça"," ",e.jsx("code",{children:"getMulti(['a', 'b', 'c', ...])"})," — uma viagem só, latência reduzida em 50x."]}),e.jsx(o,{filename:"bin/multi.php",code:`<?php
declare(strict_types=1);
...
produto:99 estava no resultado? não — não existia`}),e.jsx("h2",{children:"Counters atômicos: increment / decrement"}),e.jsx(o,{filename:"bin/contador.php",code:`<?php
declare(strict_types=1);
...
- Multi-thread em CPUs grandes          - Quer persistência ou réplica
                                        ...
                                        - Quer locks distribuídos (Redlock)`}),e.jsxs(a,{type:"info",title:"Resposta curta",children:["Em projeto novo, comece com ",e.jsx("strong",{children:"Redis"})," — ele faz tudo que Memcached faz e muito mais, com pouca diferença de performance. Vá para Memcached só se você tem um caso específico de cache massivo de página, com chaves enormes e CPUs de 32+ cores onde o multi-threading compensa."]}),e.jsx("h2",{children:"Subindo Memcached local"}),e.jsx(r,{title:"docker-compose.yml",language:"yaml",code:`services:
  memcached:
    image: memcached:1.6-alpine
    ports:
      - "11211:11211"
    command: ["memcached", "-m", "256", "-v"]`}),e.jsx(s,{user:"dev",host:"php",cwd:"~/projetos/api",command:"docker compose up -d memcached && echo stats | nc -q 1 localhost 11211 | head -8",output:`[+] Running 2/2
